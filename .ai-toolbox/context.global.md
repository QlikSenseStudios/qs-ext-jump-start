...
- **Documentation Location Map**: Each location serves a distinct audience — `.ai-toolbox/` (excluding `.ai-toolbox/docs/`) is AI agent context only (rules, domain knowledge, commands, patterns, tools); `.ai-toolbox/docs/` is human-readable documentation about the context system itself, accessible in all modes; `/docs/` is human-facing project documentation (setup, architecture, testing, contributing); `README.md` is the first-contact entry point and becomes the extension developer's primary project doc after initialization; additional extension documentation structure beyond `README.md` is the extension developer's choice
