...
**New selector pattern rule**: Before writing a selector for a Nebula Hub element not already documented here, ask the developer to inspect the DOM and confirm the selector rather than inferring it. Incorrect selectors that appear to work in isolation can produce false positives or fragile tests. Once confirmed, add the pattern to this section before closing the branch.
...
**Monaco editor — read**: `window.monaco` is not exposed in Nebula Hub, and Monaco virtualizes rows — only visible lines exist in the DOM, so collecting `.view-line` element text truncates any document beyond the visible viewport. Read via clipboard instead, symmetric with the write approach below: click `.monaco-editor .view-lines` to focus, `Control+a` to select all, `Control+c` to copy, then `page.evaluate(() => navigator.clipboard.readText())`. This targets Monaco's full document model rather than the rendered DOM, so it works regardless of document length. Requires the same `clipboard-read`/`clipboard-write` context permissions described below.
...
**Config panel — props controls**: The config panel (right sidebar) is always visible — it does not require the properties dialog to be open. Custom props controls are rendered inside nested MUI accordions. To interact with a control: (1) expand the top-level `props` accordion via its `MuiAccordionSummary-root` button; (2) expand the nested group accordion (e.g. `debug`) the same way; (3) target the control by its label text using `createMuiFormControlSelector(name, type)` from `props-structure-analyzer.js` — do **not** use `~ .MuiCollapse-root` sibling selectors, which are fragile and fail when the collapse region is not the direct sibling. For a checkbox leaf, the clickable label is `.MuiFormControlLabel-root:has(.MuiFormControlLabel-label:has-text("name"))` and the actual input is found by appending `CONFIGURATION_IDENTIFIERS.MUI_CHECKBOX_INPUT`. For a textfield leaf, the container is `.MuiTextField-root:has(.MuiInputLabel-root:has-text("name"))` and the input is appended with `CONFIGURATION_IDENTIFIERS.MUI_TEXT_INPUT`. After filling a text input, `press('Tab')` to trigger the MUI change event — Nebula Hub may not commit the value to the engine without a blur event. `createMuiAccordionSelector(name)` and `createMuiFormControlSelector(name, type)` in `props-structure-analyzer.js` are the canonical helpers for generating these selectors; use them rather than inlining the selector strings.
